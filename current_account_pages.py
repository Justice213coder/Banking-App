...
def main():
    st.title("Current Account Page")
    st.session_state.accounts = st.session_state.get('accounts', {})

    account_no = st.text_input("Account No", key="current_acc_no")
    amount = st.number_input("Amount", min_value=0.0, step=0.01, key="current_amount")
